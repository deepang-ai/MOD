...
class MODSwinUNETR(nn.Module):
    ...
    def __init__(
        ...
        self.decoder = SwinUNETRDecoder(img_size, in_channels)
    ...
    def forward(self, x_in, mask=None, return_seg=False):
        ...
        hidden_states_out = self.swinViT(x_in, mask, self.normalize)
        enc0 = self.encoder1(x_in)
        enc1 = self.encoder2(hidden_states_out[0])  # [b,24,64,64,64]
        ...
        seg_result = self.out(out)

        pred = self.decoder(hidden_states_out[-1])
        ...
        if return_seg:
            ...
            return {
                "recon": pred,
                "patch": rearrange(hidden_states_out[-1], "n c d h w -> n (d h w) c"),
                "seg": seg_result,
            }
...
class SwinTransformer(nn.Module):
    ...
    def forward(self, x, mask=None, normalize=True):
        x0 = self.patch_embed(x)  # [b,24,64,64,64]
        x0 = self.pos_drop(x0)
        if mask is not None:  # [b,64,64,64]
        ...
        x4 = self.layers4[0](x3.contiguous())  # [b,384,4,4,4]
