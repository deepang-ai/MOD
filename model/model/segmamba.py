...
class MODSegMamba(nn.Module):
    def __init__(
        ...
        self.decoder = SegMambaDecoder(out_channels=in_chans)
    ...
    def forward(self, x_in, mask=None, return_seg=False):
        ...
        pred = self.decoder(outs[-1])
        if return_seg:
            return logits
        else:
            return {
                "recon": pred,
                "patch": rearrange(outs[-1], "n c h w d -> n (h w d) c"),
                "seg": logits,
            }
