...
from model.model.vit import MODViTAutoEnc
...
class MODTrainer(object):
    def __init__(self, config: EasyDict):
        ...
        self.student_model = MODViTAutoEnc(**config.vit)

        if self.seg_only:
            self.student_model.decoder.requires_grad_(False)
            self.student_model.masked_embed.requires_grad = False
        else:
            self.teacher_model = MODViTAutoEnc(**config.vit)
        ...
        self.img_size = config.vit.img_size
        self.train_loader, self.val_loader = get_dataloader(config, config.vit.img_size)
        ...
        self.inference = monai.inferers.SlidingWindowInferer(
            roi_size=ensure_tuple_rep(self.student_model.img_size, 3),
            overlap=0.5,
    ...
    def _run(self):
        ...
        self.starting_epoch, self.train_step, self.val_step = self.resume_train_state(
            f"{os.getcwd()}/output/vit/checkpoint",
            self.train_loader,
            self.val_loader,
        )
        ...
        for epoch in range(self.starting_epoch, self.config.trainer.num_epochs):
            ...
            if mean_acc > best_acc:
                ...
                self.accelerator.save_state(
                    output_dir=f"{os.getcwd()}/output/vit/checkpoint/best"
                )

            self.accelerator.save_state(
                output_dir=f"{os.getcwd()}/output/vit/checkpoint/epoch_{epoch}"
            )
