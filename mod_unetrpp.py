...
from model.model.unetrpp import MODUNETR_PP
...
class MODTrainer(object):
    def __init__(self, config: EasyDict):
        ...
        self.student_model = MODUNETR_PP(**config.unetrpp)

        if self.seg_only:
            self.student_model.decoder.requires_grad_(False)
            self.student_model.unetr_pp_encoder.masked_embed.requires_grad = False
        else:
            self.teacher_model = MODUNETR_PP(**config.unetrpp)
        ...
        self.train_loader, self.val_loader = get_dataloader(
            config, config.unetrpp.image_size
        )
    ...
    def _run(self):
        ...
        self.starting_epoch, self.train_step, self.val_step = self.resume_train_state(
            f"{os.getcwd()}/output/unetrpp/checkpoint",
            self.train_loader,
            self.val_loader,
        )
        ...
        for epoch in range(self.starting_epoch, self.config.trainer.num_epochs):
            ...
            if mean_acc > best_acc:
                ...
                self.accelerator.save_state(
                    output_dir=f"{os.getcwd()}/output/unetrpp/checkpoint/best"
                )

            self.accelerator.save_state(
                output_dir=f"{os.getcwd()}/output/unetrpp/checkpoint1/epoch_{epoch}"
            )
