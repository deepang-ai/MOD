...
from model.model.segmamba import MODSegMamba
...
class MODTrainer(object):
    def __init__(self, config: EasyDict):
        ...
        self.student_model = MODSegMamba(**config.segmamba)

        if self.seg_only:
            self.student_model.decoder.requires_grad_(False)
            self.student_model.vit.masked_embed.requires_grad = False
        else:
            self.teacher_model = MODSegMamba(**config.segmamba)
            self.teacher_model.requires_grad_(False)
            self.teacher_model.to(self.accelerator.device)
        self.patch_size = 4
        self.embed_dim = 768
        self.img_size = 128
        self.train_loader, self.val_loader = get_dataloader(config, 128)
        ...
        self.inference = monai.inferers.SlidingWindowInferer(
            roi_size=ensure_tuple_rep(128, 3),
            overlap=0.5,
    ...
    def _run(self):
        ...
        self.starting_epoch, self.train_step, self.val_step = self.resume_train_state(
            f"{os.getcwd()}/output/segmamba/checkpoint",
            self.train_loader,
            self.val_loader,
        )
        ...
        for epoch in range(self.starting_epoch, self.config.trainer.num_epochs):
            ...
            if mean_acc > best_acc:
                ...
                self.accelerator.save_state(
                    output_dir=f"{os.getcwd()}/output/segmamba/checkpoint/best"
                )

            self.accelerator.save_state(
                output_dir=f"{os.getcwd()}/output/segmamba/checkpoint/epoch_{epoch}"
            )
