...
class MODTrainer(object):
    def __init__(self, config: EasyDict):
        ...
        self.student_model = MODSwinUNETR(use_v2=True, **config.swin_unetr)

        if self.seg_only:
            self.student_model.decoder.requires_grad_(False)
            self.student_model.swinViT.masked_embed.requires_grad = False
        else:
            self.teacher_model = MODSwinUNETR(use_v2=True, **config.swin_unetr)
    ...
    def _run(self):
        ...
        self.starting_epoch, self.train_step, self.val_step = self.resume_train_state(
            f"{os.getcwd()}/output/swin_unetrv2/checkpoint",
            self.train_loader,
            self.val_loader,
        )
        ...
        for epoch in range(self.starting_epoch, self.config.trainer.num_epochs):
            ...
            if mean_acc > best_acc:
                ...
                self.accelerator.save_state(
                    output_dir=f"{os.getcwd()}/output/swin_unetrv2/checkpoint/best"
                )

            self.accelerator.save_state(
                output_dir=f"{os.getcwd()}/output/swin_unetrv2/checkpoint/epoch_{epoch}"
            )
