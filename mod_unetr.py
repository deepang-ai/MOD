...
from model.model.unetr import MODUnetr
...
class MODTrainer(object):
    def __init__(self, config: EasyDict):
        ...
        self.student_model = MODUnetr(**config.unetr)

        if self.seg_only:
            self.student_model.decoder.requires_grad_(False)
            self.student_model.encoder.masked_embed.requires_grad = False
        else:
            self.teacher_model = MODUnetr(**config.unetr)
            self.teacher_model.requires_grad_(False)
            self.teacher_model.to(self.accelerator.device)
        self.patch_size = self.student_model.patch_size
        self.embed_dim = self.student_model.embed_dim
        self.img_size = self.student_model.image_size
        self.train_loader, self.val_loader = get_dataloader(
            config, config.unetr.image_size
        )
        ...
        self.inference = monai.inferers.SlidingWindowInferer(
            roi_size=ensure_tuple_rep(self.student_model.image_size, 3),
            overlap=0.5,
        ...
        if not self.seg_only:
            ...
            self.dp_loss = DPLoss(self.patch_size, self.config.trainer.dp_ratio)
    ...
    def _run(self):
        ...
        self.starting_epoch, self.train_step, self.val_step = self.resume_train_state(
            f"{os.getcwd()}/output/unetr/checkpoint",
            self.train_loader,
            self.val_loader,
        )
        ...
        for epoch in range(self.starting_epoch, self.config.trainer.num_epochs):
            ...
            if mean_acc > best_acc:
                ...
                self.accelerator.save_state(
                    output_dir=f"{os.getcwd()}/output/unetr/checkpoint/best"
                )

            self.accelerator.save_state(
                output_dir=f"{os.getcwd()}/output/unetr/checkpoint/epoch_{epoch}"
            )
